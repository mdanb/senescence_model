%=========================================================================%
% function run_optimization_leaveout(leaveout, fit, data, range_lb, range_ub, state_num, type, name)
% Created By: Danny Galvis, Darren Walsh, James Rankin
% This function runs the optimization
% Inputs:
%    - leaveout - This is if you want to choose one parameter not to
%    optimize (must be an index) with fmincon. If the index chosen is
%    larger than the number of parameters, this does nothing
...
%=========================================================================%
function run_optimization_leaveout(leaveout,fit, data, range_lb, range_ub, state_num, type, seed_in, output_file)
    ...
    if strcmp(type, 'ga')|| strcmp(type, 'both')
        ...
        options_ga = optimoptions(@ga,'UseParallel',true,'Display','iter',...
                            'PopulationSize',2000,'MaxGenerations',10);%,...
                            %'PlotFcn',{'gaplotrange'});
        ...
        % leaveout is the parameter that does not get optimized.
        keep = 1:length(result_mid);
        keep = keep(keep ~= leaveout);
        
        range_lb_final = result_mid;
        range_ub_final = result_mid;
        
        range_lb_final(keep) = 0.5*result_mid(keep);
        range_ub_final(keep) = min(1.5*result_mid(keep),1);
